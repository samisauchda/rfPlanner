...
class _StubEngine(PropagationEngine):
    """Minimal engine returning a flat -60 dBm layer; no dependencies needed."""
...
def test_layer_key_depends_on_all_parts():
    eng = _StubEngine()
    s1, s2 = SceneConfig(noise_figure_db=7.0), SceneConfig(noise_figure_db=10.0)
...
def test_aggregation_shapes_and_sinr():
    ...
    res = combine.aggregate(grid, scene, layers, txs, "stub")
...
def test_force_bypasses_cache():
    with tempfile.TemporaryDirectory() as d:
        sim = Simulator(SceneConfig(), GridSpec(cell_size=2.0), engine=_StubEngine(), cache=d)
        sim.place("A", 0, 0)
        sim.run()
...
def test_render_overlay_returns_data_url():
    sim = Simulator(SceneConfig(), GridSpec(cell_size=2.0), engine=_StubEngine(), cache=None)
    sim.place("A", 0, 0)
    res = sim.run()
